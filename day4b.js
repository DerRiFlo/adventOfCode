...
function printBoard(board,markers){
    score =0
    for(var x=0; x<board[0].length; ++x){
        str = ""
        for(var y=0;y<markers.length; ++y){
...
function checkLastWinner(boardWinners){
    notWinnerCount =0
    for(var i=0;i<boardWinners.length;++i){
        if (!boardWinners[i])
            ++notWinnerCount
    }
    if (notWinnerCount == 1)
...
/// Play game
boardWinners = new Array(boards.length).fill(false)
for(var round=0;round<roundNumbers.length;++round){
    for(var board=0; board<boards.length; ++board){
        if(!boardWinners[board]){
            ...
            {
                if(checkLastWinner(boardWinners))
                {
                    printBoard(boards[board],markers[board])
                    console.log("Board: " + board)
                    console.log("Last Number: " + roundNumbers[round])
                    console.log("Score: " + calcScore(boards[board],markers[board])*roundNumbers[round])
                    return
                }
                boardWinners[board] = true
            }
        }
